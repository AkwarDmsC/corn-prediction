...
from v7.config import CORE_WEIGHTS, ELIMINATED_SIGNALS, VERSION
...
def compute_signal_snapshot(
    ...
    *,
    ...
    # ── 新增做多因子：布林下轨反弹 ──
    bb_lower_touch_sig = 0.0
    if bb_pos < 20 and ma_dir <= 0:
        # 价格跌到布林下轨且均线不显著多头 → 可能的反弹
        bb_lower_touch_sig = 1.0
    elif bb_pos < 20:
        # 布林下轨但均线多头 → 弱势震荡，反弹概率降低
        bb_lower_touch_sig = 0.3
    ...
    if len(closes) >= 5:
        for j in range(1, 6):
            ...
            if closes[-j] < closes[-j-1]:
                ...
                break
    consec_down_sig = 1.0 if consec_down >= 3 else 0.0

    # ── 新增做多因子：跳空低开（缺口修复） ──
    gap_down_sig = 0.0
    if len(closes) >= 2:
        gap_pct = (float(df.iloc[-1]["open"]) - float(df.iloc[-2]["close"])) / float(df.iloc[-2]["close"]) * 100
        gap_down_sig = 1.0 if gap_pct < -0.3 else 0.0
    ...
    raw_weighted = [
        ...
        ("BB_LOWER_TOUCH", bb_lower_touch_sig, CORE_WEIGHTS.get("BB_LOWER_TOUCH", 1.5)),
        ("CONSECUTIVE_DOWN", consec_down_sig, CORE_WEIGHTS.get("CONSECUTIVE_DOWN", 0.8)),
        ("GAP_DOWN", gap_down_sig, CORE_WEIGHTS.get("GAP_DOWN", 0.5)),
    ]
    ...
    return {
        ...
        "indicators": {
            ...
            "consecutive_down": consec_down,  # v7.1 新增
        },
        ...
        "bb_lower_touch_sig": bb_lower_touch_sig,  # v7.1 新增
        "consecutive_down_sig": consec_down_sig,    # v7.1 新增
        "gap_down_sig": gap_down_sig,                # v7.1 新增
    }
